#!/usr/bin/env python3
"""Canonical theme, navigation, and shared front-end assets for Discipline GitHub Pages site.

Single source of truth consumed by build_apt_repo.py and build_rpm_repo.py.
Follows the Ecosystem Theme Contract v1:
...
THEME_CSS_VARS = (
    "\n"
    + _vars_block(":root", DARK_PALETTE, "dark")
    + "\n\n"
    + _vars_block('[data-theme="light"]', LIGHT_PALETTE, "light")
    + "\n"
)
...
THEME_HEAD_JS_BODY = """    (function() {
      'use strict';
      if (window.__disciplineApplyTheme) { return; }
      var KEY = 'orieg-theme';
      ...
      function apply() {
        ...
        if (window.__disciplineThemeChanged) { window.__disciplineThemeChanged(theme); }
      }
      window.__disciplineApplyTheme = apply;
      apply();
...
THEME_TOGGLE_JS_BODY = """    function toggleTheme() {
      ...
      if (window.__disciplineApplyTheme) { window.__disciplineApplyTheme(); }
    }
"""
...
def make_nav(version: str, active: str, base: str = "./") -> str:
    """Renders the shared navbar. active in {home, capabilities, platforms, gates, architecture, install, apt, rpm};
    base is './' for root-level pages and '../' for apt/rpm subpages."""
    links = [
        ("home", base, "Home"),
        ("gates", f"{base}#gates", "Gates"),
        ("platforms", f"{base}#platforms", "Platforms"),
        ("architecture", f"{base}#architecture", "Architecture"),
        ("install", f"{base}#install", "Install"),
        ("apt", f"{base}apt/", "APT"),
        ("rpm", f"{base}rpm/", "RPM"),
    ]
    items = "\n".join(
    ...
    <nav class="nav-scroll" aria-label="Site">
      <ul class="nav-links">
{items}
        <li><a href="https://github.com/orieg/discipline/blob/main/docs/ARCHITECTURE.md">Docs</a></li>
        <li><a href="https://github.com/orieg/discipline" class="nav-pill">GitHub &bull; v{version}</a></li>
      </ul>
