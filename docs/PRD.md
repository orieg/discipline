...
hostname_denylist = []          # whole-token, case-insensitive, never echoed (active config is exempt from flagging itself; secrets are best passed via DISCIPLINE_HOSTNAME_DENYLIST)
...
| JavaScript / TypeScript | `test(` / `it(` callbacks (Jest, Vitest, Mocha, node:test) | `expect(…).matcher`, `assert.*`; strong: `toBe`, `toEqual`, `toStrictEqual`, `deepStrictEqual`; weak: `toBeTruthy`, `toBeDefined` | `.skip`, `.todo`, `xit`, `xdescribe`, `.only` (narrows the suite) | `@ts-ignore`, `@ts-expect-error`, `as any`, `eslint-disable*` | **shipped** |
...
| **3 — Language packs I** | extractor trait and per-pack test identity split out of the Rust-shaped matcher; Python and JavaScript / TypeScript packs; grammars as compile-time features | each pack passes §9 items 1–4 in its own language, including a callback-style test renamed without changing its body | **shipped** |
...
- **CI verified on GitHub and Gitea.** All 11 workflow jobs (`ci-gate`, `dogfood`, `test` on Ubuntu and macOS, `lint`, `msrv`, `supply-chain`, `build-static`, `action-github`, `action-gitea`, `pre-commit`) pass on pull request branches against main.
...
- **GitLab CI and Argo templates have not executed on live runners.** `.gitlab-ci.yml`, `templates/discipline.gitlab-ci.yml`, and `templates/argo-workflow-template.yaml` are validated and linted offline in CI, but have never executed against live GitLab CI runners or a live Argo cluster.
- **Language pack detection boundaries:** Shipped packs (Rust, Python, JavaScript / TypeScript, PHPT) pass F3–F6. What is NOT detected yet per pack:
  - *Rust:* Macro-generated tests are opaque (`proptest! { ... }`, `quickcheck! { ... }`); custom assert macros need registration in `extra_assert_macros` / `assert_helper_fns`. Assertion strength uses a two-level proxy (equality/pattern vs. general).
  - *Python:* Dynamic `@pytest.mark.parametrize` expansions (counted per definition, not per expansion); assertions inside helper functions unless declared in `assert_helper_fns`; class inheritance outside `unittest.TestCase`; dynamic runtime skip calls (`pytest.skip(...)`) inside test bodies.
  - *JavaScript / TypeScript:* Programmatic test generators (`test.each(...)`, `forEach` loops); custom matchers unless declared in `assert_helper_fns` / `extra_assert_macros`; dynamic conditional skips (early `return`).
  - *Golden (PHPT):* Dynamic runtime PHP logic inside `--FILE--` or `--SKIPIF--` sections; multiple logical assertions inside a single `.phpt` file.
- **Grammar lag.** Syntax newer than the bundled tree-sitter grammar reports as a parse error, which blocks by design; `exempt_paths` is the escape.
