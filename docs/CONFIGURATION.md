...
```yaml
include:
  - remote: 'https://raw.githubusercontent.com/orieg/discipline/v0.12.2/templates/discipline.gitlab-ci.yml'
```
...
`--json` prints the per-change verdicts and the per-gate counts (`errors_by_gate` names the changes each gate blocked; `could_not_check_by_reason` groups the changes that could not be checked by the error they stopped on). The command exits 0 when the replay ran, whatever it found; 2 when it could not run.
...
- `ghcr.io/orieg/discipline:v0.12.2`
...
2. **Job container, no `uses:` (Gitea Actions, Forgejo Actions, GitHub Actions):**
   ...
   jobs:
     discipline:
       ...
       container:
         image: ghcr.io/orieg/discipline:v0.12.2@sha256:<digest of that release>
       defaults:
         run:
