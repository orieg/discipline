...
**The promise is a test.** `tests/fixtures/v1_surface.json` lists every frozen name: subcommands, flags and their values, the `DISCIPLINE_*` variables, configuration keys, directives, gate ids, finding codes, could-not-check reasons, MCP tools and arguments, hook agents, and the action's inputs and outputs. `tests/test_stability_contract.rs` collects each from the code and fails when a recorded name is gone; a new name must be recorded (`DISCIPLINE_BLESS_SURFACE=1 cargo test --test test_stability_contract`). The JSON outputs are pinned field by field in `tests/test_output_schemas.rs`.
