...
- **Mode:** Rust is analysed with `build-mode: autobuild`: CodeQL runs the cargo build itself and extracts from it, so build-script output and proc-macro expansions come from the real compiler rather than from source-only extraction. `actions` and `python` have no build step and stay on `build-mode: none`.
- **Why not `none`:** Source-only extraction produced alert `rust/cleartext-logging` (CWE-312) on `src/report/mod.rs`, triaged as a false positive: no report format echoes a detected secret, and `tests/test_report_redaction.rs` pins redaction across every output format. A built extraction is the more precise model, so it is the default; the alert delta between the two modes is recorded in the pull request that switched them.
