...
| Reports | The JSON report's existing fields, their names and types (`--format json`, `replay --json`), as `discipline.report.schema.json` and `discipline.replay.schema.json` define them (generated by `discipline docs`; `tests/test_output_schemas.rs` pins every field and validates real output); consumers ignore fields they do not know; SARIF 2.1.0, JUnit and GitLab Code Quality follow their external schemas | New fields; terminal, Markdown and agent-prompt text |
