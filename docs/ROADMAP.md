...
- **Status:** shipped in v0.10.0 (Steps 0-5), with one row partial: the forge-side `doctor` step runs in CI but not under `--strict` until the default branch's rules block deletion and require pull requests (owner settings). Order followed: Step 0 (correctness for a live consumer), Step 1 (it widened what every later step covers), Step 2, Step 5 (a second consumer-replay correctness item, ahead of coverage work), Step 3, Step 4.
...
- **Forge-side `doctor` under `--strict`:** the CI step runs with the workflow's read token and reports two Warns that are repository settings (deletion not blocked, direct pushes allowed); `bypass` is Warn because the bypass list is shown to admin tokens only. `--strict` is switched on once the rules change.
